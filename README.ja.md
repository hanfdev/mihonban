...
- 永続的な再生キュー、ユーザー操作内で開始するモバイル再生、シャッフル／リピート、Range シーク、Media Session
...
Windows では、補助スクリプトが OneDrive の外へビルドファイルを配置し、すべてのネットワークインターフェースで Wrangler を起動します。

```powershell
tools\cloud-dev.cmd
```

`http://127.0.0.1:8787` を開きます。同じ LAN 内のスマートフォンからは、Windows ファイアウォールで Node.js を許可したうえで `http://<computer-lan-ip>:8787` にアクセスできます。初回に生成されるローカルシークレットでは、リスナーパスワードが `mihonban-guest`、管理者パスワードが `mihonban-admin` です。サービスを共有する前に、管理画面で両方を変更してください。
