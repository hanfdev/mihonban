...
|データ |移行経路 |
|---|---|
|アルバム、トラック、アーティストと旧名の対応、ギャラリー、お気に入り、ノート、ソース投稿 |D1 SQLエクスポート/インポート |
...
アルバムの順序付きアーティスト表記は `album_artists` に保存されます。曲固有の表記だけが `track_artists` に保存され、行がない曲はアルバム表記を継承します。論理 SQL エクスポートには両方のテーブルが含まれます。アップグレード後の初回利用時に Mihonban がテーブルを自動作成し、既存アルバムのアーティスト文字列をそのまま 1 件のクレジットとして補完します。カンマは名前やソート名にも使われるため、自動分割はしません。全編のコラボレーションはアルバム編集、特定曲だけのゲストは「曲を管理」のアーティストボタンで設定してください。

`artist_sort` は任意項目です。空欄はエクスポートとインポートでも保持され、検索と並べ替えの実行時だけ原名へフォールバックします。
