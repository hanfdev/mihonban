...
| 数据 | 迁移方式 |
|---|---|
| 专辑、曲目、艺人及旧名称关联、内页、收藏、备注、资源站状态 | D1 SQL 导出/导入 |
...
专辑与艺人的有序关系保存在 `album_artists`；只有某首歌具有独立署名时才写入 `track_artists`，没有歌曲级记录就自动继承专辑艺人。逻辑 SQL 导出会包含这两张表。升级后首次使用时，Mihonban 会自动建表，并把每张旧专辑的原艺人字段原样回填为一位艺人。系统不会按逗号自动拆分，因为逗号也可能属于艺人名或排序名。整张合作请使用专辑编辑器；只有部分歌曲出现的客席艺人，请在“管理曲目”的艺人按钮中设置。

`artist_sort` 是可选值：导出和导入会保留空值，搜索与排序仅在运行时回退使用艺人原名。
