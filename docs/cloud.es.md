...
Dentro de un mismo almacenamiento con nombre, la identidad de las carpetas de álbum y las rutas de pista no distingue mayúsculas y minúsculas. Las rutas guardadas se conservan y se rechazan los duplicados posteriores que solo difieran en el uso de mayúsculas.

La identidad de los artistas no distingue mayúsculas y minúsculas en todo el catálogo. Los administradores pueden cambiar el nombre completo desde el perfil o corregir mayúsculas y minúsculas en los créditos de álbumes y pistas. Una sola transacción actualiza la ficha del artista, todos los créditos, los créditos de importación guardados, las notas y las correspondencias de nombres anteriores. Se conservan el perfil, los favoritos, los identificadores de álbumes y pistas y las rutas de almacenamiento. Los nombres anteriores apuntan directamente al artista actual para los enlaces y las importaciones; los cambios sucesivos no crean cadenas de alias. Los conflictos con un artista existente o con el nombre anterior de otro artista devuelven HTTP 409 sin fusionar fichas. La actualización en tiempo de ejecución añade la tabla indexada `artist_aliases`, incluida en las exportaciones lógicas de SQLite para conservar los enlaces anteriores y la correspondencia de las importaciones tras restaurar los datos.
