...
카탈로그 전체의 아티스트도 대소문자를 구분하지 않고 동일성을 판단하며, 가져오기와 재검색에서는 저장된 표시 이름을 사용합니다. 관리자는 앨범 또는 곡 편집기에서 기존 아티스트 이름의 대소문자를 바로잡을 수 있습니다. 하나의 트랜잭션으로 아티스트 레코드, 앨범과 곡의 크레딧, 저장된 가져오기 크레딧, 아티스트 메모를 갱신하며, 이후 가져오기에서도 수정된 이름을 사용합니다. 프로필, 즐겨찾기, 앨범과 곡의 ID, 저장 경로는 유지됩니다. 기존 데이터에 대소문자만 다른 아티스트 레코드가 충돌하면 자동으로 병합하지 않고 HTTP 409를 반환합니다. 대소문자를 수정하기 위한 데이터베이스 스키마 마이그레이션은 필요하지 않습니다.
