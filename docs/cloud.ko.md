...
같은 이름 있는 저장소 안에서는 앨범 폴더와 트랙 경로의 대소문자를 구분하지 않고 동일성을 판단합니다. 저장된 경로는 유지하며, 대소문자만 다른 이후의 중복 항목은 거부합니다.

카탈로그 전체의 아티스트는 대소문자를 구분하지 않고 동일성을 판단합니다. 관리자는 프로필 페이지에서 전체 이름을 변경하거나 앨범과 곡의 크레딧에서 대소문자를 바로잡을 수 있습니다. 하나의 트랜잭션으로 아티스트 레코드, 모든 크레딧, 저장된 가져오기 크레딧, 메모, 이전 이름 연결을 갱신합니다. 프로필, 즐겨찾기, 앨범과 곡의 ID, 저장 경로는 유지됩니다. 이전 이름은 링크와 가져오기에서 현재 아티스트로 직접 연결되며, 반복해서 이름을 바꿔도 별칭이 연쇄적으로 연결되지 않습니다. 기존 아티스트 이름이나 다른 아티스트의 이전 이름과 충돌하면 병합하지 않고 HTTP 409를 반환합니다. 런타임 업그레이드에서 인덱스가 있는 `artist_aliases` 테이블을 추가하며, SQLite 논리 내보내기에도 포함하므로 복원 후에도 이전 링크와 가져오기 연결이 유지됩니다.
