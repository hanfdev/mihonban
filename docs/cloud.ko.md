...
트랙 제목의 원본은 명시적으로 관리됩니다. 관리 화면에서 트랙 이름을 바꾸면 `tracks.title_override` 플래그가 설정되며, 이후 로컬 컴패니언 스캔, 앨범 전체 재등록, 단일 트랙 동기화에서도 해당 제목을 유지합니다. 일반 트랙은 계속 파일 태그를 따릅니다. 기존 D1에는 이 열이 자동으로 추가됩니다. 마이그레이션 이전 행은 다음 등록 시 분류됩니다. 값이 같으면 일반 동기화 제목이 되고, 다르면 데이터 손실을 막기 위해 현재 D1 제목을 수동 변경으로 보존합니다. SQLite/D1 논리 백업에도 이 플래그가 유지됩니다.
...
- 웹 업로드는 저장소 백엔드가 보고한 정확한 바이트 수가 예상값과 같을 때만 승인됩니다. 재개 가능한 공급자는 일시적 오류 뒤에 확정된 오프셋을 조회해 이어서 보내고, 프록시형 공급자는 요청 본문 전체를 다시 보냅니다.
...
앨범 목록과 작은 아트워크 영역은 Cloudflare Image Transformations로 저장된 원본에서 256 px 또는 640 px WebP 파생 이미지를 만듭니다. `fit: scale-down`은 갱신이 늦을 수 있는 저장소 썸네일 캐시에 의존하지 않고 수동 또는 Discogs에서 정한 구도를 그대로 유지하며, 스크롤 중 수 MB 원본을 디코딩하는 비용을 피합니다. 상세 보기와 자르기 화면은 계속 원본을 사용합니다. R2는 변환 원본인 `art:<album-id>:original`을 유지하면서 `art:<album-id>:256`과 `art:<album-id>:640`도 저장합니다. 변환을 사용할 수 없으면 API가 원본을 대신 제공하지만 파생 이미지 키에는 기록하지 않습니다.
...
Worker는 API 요청을 처리하기 전에 오래된 카탈로그를 업그레이드합니다. 공개 Wrangler 템플릿은 안정적인 `DB_SCHEMA_KEY`를 제공하며, 업그레이드가 성공하면 현재 코드의 런타임 버전을 `settings.schema_version`에 기록합니다. 같은 isolate의 후속 요청은 메모리 상태를 사용하고, 콜드 스타트 isolate도 이 한 행만 읽은 뒤 호환 DDL과 백필을 건너뜁니다. `DB_SCHEMA_KEY`를 안정적으로 유지하고 이 마커를 일반 정리 작업으로 삭제하지 마세요. 릴리스에 런타임 마이그레이션을 추가할 때는 유지 관리자가 `RUNTIME_SCHEMA_VERSION`도 올려야 합니다.

같은 이름 있는 저장소 안에서는 앨범 폴더와 트랙 경로의 대소문자를 구분하지 않고 동일성을 판단합니다. 카탈로그 전체의 아티스트도 같은 규칙을 따릅니다. 처음 저장된 표시 철자는 보존하지만 대소문자만 다른 이후의 중복 항목은 거부합니다.

Workers Free의 D1은 현재 하루 500만 행 읽기와 10만 행 쓰기를 포함하며 00:00 UTC에 초기화됩니다. 재배포나 코드 수정으로 이미 집계된 사용량을 줄일 수 없고, 쿼리가 다시 동작한 뒤에도 대시보드 집계가 이전 합계를 계속 표시할 수 있습니다. 현재 서비스 상태는 캐시를 쓰지 않는 API 요청이나 직접 D1 쿼리와 각 쿼리의 `meta.rows_read` / `meta.rows_written`으로 확인하세요. 대시보드 막대는 사용 이력이지 실시간 잠금 표시가 아닙니다. 사용량이 다시 빠르게 증가하면 배포 설정에 `DB_SCHEMA_KEY`가 남아 있고 `settings.schema_version`이 현재 Worker와 일치하는지 확인하세요. 최신 한도는 [D1 가격 문서](https://developers.cloudflare.com/d1/platform/pricing/)를 참고하세요.
