...
- `album_images`：位於專輯後端的內頁路徑，以及用於保證外部圖片匯入冪等性的可選穩定來源識別。
...
曲目標題的權威來源是明確的：管理員在網頁中重新命名曲目後，系統會設定 `tracks.title_override` 標記；後續本機伴侶掃描、整張專輯登記和單曲同步都會保留該標題，普通曲目則繼續跟隨檔案標籤。舊 D1 會自動增加此欄位。遷移前的曲目會在下一次登記時完成歸類：標題一致則作為普通同步標題，不一致則以避免遺失資料為原則，保留目前 D1 標題並認定為人工修改。SQLite/D1 邏輯備份會保留該標記。
...
- 網頁上傳只有在儲存後端回報的精確位元組數與預期一致後才會被接受。可續傳服務會在短暫故障後查詢已提交偏移量繼續上傳；代理型服務則重試完整請求主體。
...
公開 R2 圖片重新導向會在瀏覽器與 Cloudflare 邊緣快取 5 分鐘，並啟用 stale-while-revalidate。重新導向目標是帶版本號且不可變的 R2 URL，因此重新整理曲庫時不會為每張封面重複呼叫 Worker；更換封面也能在快取視窗後及時生效。隱藏圖片與音訊重新導向仍維持私人且不快取。

專輯列表與緊湊封面區域會透過 Cloudflare Image Transformations，從儲存中的來源檔案產生 256 px 或 640 px WebP 衍生圖。`fit: scale-down` 會保留手動或 Discogs 裁切後的準確構圖，不依賴可能延遲更新的儲存服務縮圖快取，同時避免捲動時解碼數 MB 的原圖。詳情與裁切介面仍使用原圖。R2 會保留 `art:<album-id>:original` 原圖轉換來源，並同時儲存 `art:<album-id>:256` 與 `art:<album-id>:640`。轉換不可用時，API 會退回來源圖，但不會把來源圖錯誤登記到衍生圖鍵下。
...
Worker 會在處理 API 請求前升級舊曲庫。公開 Wrangler 範本提供穩定的 `DB_SCHEMA_KEY`，升級成功後會把目前程式碼的執行時版本寫入 `settings.schema_version`。同一 isolate 後續直接使用記憶體狀態；冷啟動 isolate 只讀取這一行標記，隨後略過相容 DDL 和資料回填。請保持 `DB_SCHEMA_KEY` 穩定，也不要把該標記當作一般垃圾資料刪除。後續版本若增加執行時遷移，維護者必須同時遞增 `RUNTIME_SCHEMA_VERSION`。

在同一個命名儲存內，專輯資料夾與曲目路徑按不區分大小寫的身分判斷。系統保留既有儲存路徑，並拒絕之後僅大小寫不同的重複記錄。

整個曲庫中的藝人身分不區分大小寫。管理員可在藝人資料頁修改完整名稱，也可在專輯或曲目署名中更正大小寫。系統透過同一交易更新藝人記錄、所有署名、已儲存的匯入署名、藝人備註及舊名稱關聯，保留藝人資料、收藏、專輯與曲目 ID 和儲存路徑。舊名稱會直接指向目前藝人，用於相容舊連結和後續匯入；連續更名不會形成多層別名鏈。如果新名稱與既有藝人或其他藝人的舊名稱衝突，編輯會回傳 HTTP 409，不會自動合併記錄。執行階段升級會新增帶索引的 `artist_aliases` 表，SQLite 邏輯匯出也會保留它，確保還原後舊連結和匯入比對繼續有效。

Workers Free 目前每天包含讀取 500 萬行、寫入 10 萬行的 D1 額度，並在 00:00 UTC 重設。重新部署或修復程式碼無法扣除已經計入的用量，控制台彙總也可能在查詢恢復後繼續顯示上一週期的總數。判斷目前服務狀態時，應發出不使用快取的 API 請求或直接執行 D1 查詢，並檢視每條查詢的 `meta.rows_read` / `meta.rows_written`；控制台進度條應視為歷史用量，而不是即時封鎖開關。如果用量再次快速攀升，請確認部署設定仍包含 `DB_SCHEMA_KEY`，且 `settings.schema_version` 與目前 Worker 一致。最新額度以 [D1 定價文件](https://developers.cloudflare.com/d1/platform/pricing/)為準。
