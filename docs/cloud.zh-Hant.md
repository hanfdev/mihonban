...
整個曲庫中的藝人身分也不區分大小寫，匯入和重新掃描會沿用已儲存的顯示名稱。管理員可在專輯或曲目編輯器中更正既有藝人名稱的大小寫；系統透過同一交易更新藝人記錄、專輯與曲目署名、已儲存的匯入署名及藝人備註，之後的匯入沿用更正後的名稱。藝人資料、收藏、專輯與曲目 ID 和儲存路徑均會保留。如果舊資料中存在僅大小寫不同的衝突藝人記錄，編輯會回傳 HTTP 409，不會自動合併這些記錄。大小寫更正無需資料庫結構遷移。
