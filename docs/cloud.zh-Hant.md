...
專輯封面會直接讀取儲存中的原始檔案。對手動或 Discogs 裁切的封面而言，這點很重要：服務商產生的 `c480x480` 與 `c1000x1000` 縮圖可能採用不同取景，還會再次裁切直幅原圖。因此所有封面展示共用 `art:<album-id>:original` 映象，由瀏覽器縮放同一份正方形構圖，不再要求服務商二次裁切。
