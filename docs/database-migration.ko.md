...
| 데이터 | 마이그레이션 경로 |
|---|---|
| 앨범, 트랙, 아티스트와 이전 이름 연결, 갤러리, 즐겨찾기, 노트, 소스 게시물 | D1 SQL 내보내기/가져오기 |
...
앨범의 순서 있는 아티스트 크레딧은 `album_artists`에 저장됩니다. 곡별 크레딧은 필요한 경우에만 `track_artists`에 저장되며, 행이 없는 곡은 앨범 크레딧을 상속합니다. 논리 SQL 내보내기에는 두 테이블이 모두 포함됩니다. 업그레이드 후 처음 사용할 때 Mihonban이 테이블을 만들고 기존 앨범 아티스트 문자열 전체를 하나의 크레딧으로 채웁니다. 쉼표는 이름과 정렬 이름에도 쓰이므로 자동 분리하지 않습니다. 앨범 전체 협업은 앨범 편집기에서, 일부 곡의 게스트는 곡 관리의 아티스트 버튼에서 설정하십시오.

`artist_sort`는 선택 값입니다. 빈 값은 내보내기와 가져오기에서도 유지되며 검색 및 정렬 시에만 원래 이름으로 대체됩니다.
