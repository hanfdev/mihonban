...
- La reproducción comienza dentro del toque/clic original para que Android Chrome pueda establecer audio y una sesión multimedia del sistema. En navegadores compatibles, la pantalla de bloqueo/notificación permite reproducir, pausar, cambiar de pista y buscar. Si iOS Safari agota el audio almacenado en búfer, Mihonban congela la línea de tiempo multimedia del sistema y solo la reanuda cuando el audio vuelve a poder reproducirse, para evitar que el progreso silencioso se adelante al sonido.
...
- Las cuadrículas de álbumes precargan las portadas cercanas a la ventana y mantienen un marcador discreto hasta que termina la decodificación. Las portadas ya decodificadas permanecen preparadas al filtrar u ordenar, y los fallos transitorios de imagen activan un número reducido de reintentos con demora. En la biblioteca de escritorio con puntero preciso, las portadas en reposo se atenúan ligeramente y recuperan sus colores originales al pasar el puntero, recibir el foco del teclado o estar reproduciéndose. Las tasas de bits de 1000 kbps o más usan una notación compacta en `M`; pasa el puntero sobre el valor para ver los kbps exactos.
...
El editor del álbum define el crédito ordenado de artistas predeterminado para todo el lanzamiento. En la gestión de pistas, el botón de artista junto a una canción permite definir una colaboración propia de esa pista; desactiva la opción para volver a heredar los artistas del álbum. Estos créditos se usan en la búsqueda, el reproductor y la información multimedia del sistema. La página de un invitado muestra solo las canciones en las que participa y no le atribuye el álbum completo. El complemento y el importador del navegador priorizan los valores múltiples estructurados `ARTISTS` / `ALBUMARTISTS`. Si solo existe texto de presentación, pueden separar marcas explícitas como `feat.` / `ft.` / `featuring`; las comas y `&` solo se dividen cuando varios identificadores de artista de MusicBrainz confirman el mismo número de créditos. Los nombres oficiales ambiguos, como `Neil & Iraiza`, permanecen intactos.

Los administradores pueden elegir **Editar nombre** junto al nombre en la página del artista para cambiar el nombre completo o corregir mayúsculas y minúsculas. Al guardar, el cambio se aplica al mismo artista en toda la biblioteca, incluidos los álbumes ocultos y las colaboraciones, y se conservan el perfil, los favoritos y las rutas de almacenamiento. Los nombres anteriores siguen funcionando en los enlaces del artista, la búsqueda de la lista Artistas y las importaciones posteriores. No se puede guardar un nombre usado por otro artista; los artistas nunca se fusionan automáticamente.

También se pueden corregir mayúsculas y minúsculas en los créditos de álbumes y pistas. El editor indica el alcance del cambio antes de guardar. Elegir otro artista solo cambia ese crédito.
...
Mihonban no automatiza las solicitudes a Rate Your Music. Guarda manualmente una página de lanzamiento en el navegador, importa el HTML guardado en la página del álbum y valora las críticas, el recuento de votos, géneros primarios/secundarios y descriptores antes de guardar. La CLI puede analizar, emparejar y escribir manualmente las páginas guardadas en bloque. Reimportar una página más reciente actualiza la nota, el recuento de votos y el puesto; los géneros y descriptores se fusionan con las listas existentes mediante deduplicación que ignora mayúsculas y minúsculas: las etiquetas editadas a mano o importadas de Discogs nunca se eliminan, y repetir una importación deja las listas intactas.
