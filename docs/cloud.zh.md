...
在同一个命名存储内，专辑文件夹和曲目路径按不区分大小写的身份判断。系统保留已有存储路径，并拒绝之后仅大小写不同的重复记录。

整个曲库中的艺人身份不区分大小写。管理员可在艺人资料页修改完整名称，也可在专辑或曲目署名中更正大小写。系统通过同一事务更新艺人记录、所有署名、已保存的导入署名、艺人备注及旧名称关联，保留艺人资料、收藏、专辑与曲目 ID 和存储路径。旧名称会直接指向当前艺人，用于兼容旧链接和后续导入；连续更名不会形成多层别名链。如果新名称与已有艺人或其他艺人的旧名称冲突，编辑会返回 HTTP 409，不会自动合并记录。运行时升级会添加带索引的 `artist_aliases` 表，SQLite 逻辑导出也会保留它，确保恢复后旧链接和导入匹配继续有效。
