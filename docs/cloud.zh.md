...
整个曲库中的艺人身份也不区分大小写，导入和重扫描会沿用已保存的显示名称。管理员可在专辑或曲目编辑器中更正已有艺人名称的大小写；系统通过同一事务更新艺人记录、专辑与曲目署名、已保存的导入署名及艺人备注，之后的导入沿用更正后的名称。艺人资料、收藏、专辑与曲目 ID 和存储路径均会保留。如果旧数据中存在仅大小写不同的冲突艺人记录，编辑会返回 HTTP 409，不会自动合并这些记录。大小写更正无需数据库结构迁移。
