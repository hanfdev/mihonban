...
L’identité des artistes est également insensible à la casse dans tout le catalogue. Les imports et les nouvelles analyses réutilisent le nom d’affichage enregistré. Les administrateurs peuvent corriger les majuscules et minuscules d’un artiste existant dans l’éditeur d’album ou de morceau ; une seule transaction met à jour la fiche artiste, les crédits des albums et des morceaux, les crédits d’import enregistrés et les notes de l’artiste. Les imports suivants utilisent le nom corrigé. Le profil, les favoris, les identifiants des albums et des morceaux et les chemins de stockage sont conservés. Si des données anciennes contiennent des fiches artiste concurrentes qui ne diffèrent que par la casse, la modification renvoie HTTP 409 sans les fusionner. Ces corrections ne nécessitent aucune migration du schéma de la base de données.
