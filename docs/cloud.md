...
Artist identities are also case-insensitive across the catalog. Imports and rescans reuse the saved display name. Administrators can correct an existing artist's capitalization in the album or track editor; one transaction updates the artist record, album and track credits, stored import credits, and artist notes. The corrected name is then used by future imports. Profiles, favorites, album and track IDs, and storage paths are preserved. If older data contains conflicting artist records that differ only by case, the edit returns HTTP 409 without merging those records. Capitalization corrections do not require a database schema migration.
