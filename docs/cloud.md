...
Within one named storage backend, album folders and track paths use case-insensitive identity. Stored paths are preserved, and later duplicates that differ only by letter case are rejected.

Artist identities are also case-insensitive across the catalog. Administrators can rename an artist from its profile page or correct capitalization in album and track credits. One transaction updates the artist record, all credits, stored import credits, artist notes, and previous-name mappings. Profiles, favorites, album and track IDs, and storage paths are preserved. Previous names resolve directly to the current artist for links and imports, and repeated renames do not create alias chains. Conflicts with an existing artist or another artist's previous name return HTTP 409 without merging records. Runtime upgrades add the indexed `artist_aliases` table; logical SQLite exports include it so old links and import matching survive a restore.
